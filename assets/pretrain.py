...
def get_model(model_name, root_path=default_root_path):
    ...
    if model_name == 'wiki80_cnn_softmax':
        ...
        rel2id = json.load(open('/home/sreyang/scratch/utkarsh-aclm-sigir/re/dynamic-bart-pretraining-script/ckpt/umls-rel2id.json'))
        if 'entity' in model_name:
            sentence_encoder = encoder.BERTEntityEncoder(
                max_length=128,
                pretrain_path='michiyasunaga/BioLinkBERT-large',
                mask_entity=False
            )
        else:
            sentence_encoder = encoder.BERTEncoder(
        ...
        m = model.SoftmaxNN(sentence_encoder, len(rel2id), rel2id)
        m.load_state_dict(torch.load('/home/sreyang/scratch/utkarsh-aclm-sigir/re/dynamic-bart-pretraining-script/ckpt/umls-large.pth.tar', map_location='cpu')['state_dict'])
        return m